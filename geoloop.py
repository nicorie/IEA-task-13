    ...
    def analysis1axis(self, trackerdict=None, singleindex=None, accuracy='low',
        ...
        for index in trackerkeys:   # either full list of trackerdict keys, or single index
            ...
            try:  # look for missing data
                ...
                frontscanind2, backscanind2 = analysis.moduleAnalysis(scene=scene, modWanted=modWanted, 
                                                rowWanted=rowWanted, 
                                                sensorsy=1, 
                                                sensorsx=sensorsx, 
                                                modscanfront=modscanfront, modscanback=modscanback,
                                                relative=relative, debug=debug)
                analysis.analysis(octfile=octfile,name=name,frontscan=frontscanind2,backscan=backscanind,accuracy=accuracy) 
...
class AnalysisObj:
    ...
    def analysis(self, octfile, name, frontscan, backscan,
        ...
        print(linepts.encode())
        linepts_string = str(linepts.encode())
        linepts_strip = linepts_string.strip('b')
        linepts_strip = linepts_strip.strip('\'')
        linepts_split = linepts_strip.split('\\r')
        print('hi1')        
        old_point = linepts_split[10].split(' ')
        left_point = linepts_split[20].split(' ')
        vector_x = float(left_point[0])-float(old_point[0])
        vector_z = float(left_point[2])-float(old_point[2])
        print('hi2')
        x = sp.symbols('x')
        radius = 0.145+0.115
        print(radius)
        ...
        new_point_x = float(old_point[0]) + sol[0] * (-vector_z)
        new_point_z = float(old_point[2]) + sol[0] * vector_x
        linepts_split_split = linepts_split[10].split(' ')
        
        linepts_split[10] = str(new_point_x) + ' ' + linepts_split_split[1] + ' ' + str(new_point_z) + ' ' + linepts_split_split[3] + ' ' + linepts_split_split[4] + ' ' + linepts_split_split[5] + ' '

        line_pts = ""
        for line_pts_line in range(0,21):
            line_pts = line_pts + linepts_split[line_pts_line] + "\r"
        print(line_pts.encode())

        backDict = self._irrPlot(octfile, line_pts, name+'_Back',
                                   plotflag=plotflag, accuracy=accuracy)
...
#choose Module type
system_setup = 1 # 1=1P, 2=2P, 3=2TTgap
TT_shape = 3 # 1=Round, 2=Octagonal, 3=Square

#Get lat/lon info
BSA_WSA_lat_lon = pd.read_pickle('BSA_WSA_lat_lon_1700loc_monthly.pkl')
Stations = BSA_WSA_lat_lon.iloc[:,24:27]
Stations = Stations.iloc[0:341,:]
BSA_WSA_monthly = BSA_WSA_lat_lon.iloc[:,0:24]*0.001

for i in range(len(Stations)):

    #create folder for simulations
    if i == 0:
        testfolder = os.path.abspath(f'Simulation_Tests/Geoloop/lat_lon1')  
    else:
        testfolder = os.path.abspath(f'../lat_lon1')  
    ...
    demo = RadianceObj(f'sim_{Stations["lat"][i]}_{Stations["lon"][i]}', path = str(testfolder))  

    'Weather'
    # Pull in meteorological data using pyEPW for any global lat/lon
    epwfile = demo.getEPW(lat = Stations["lat"][i], lon = Stations["lon"][i])  # Risø location as example
    # Read in the weather data pulled in above. 
    metdata = demo.readWeatherFile(weatherFile = epwfile)#, starttime='2022-06-01', endtime='2022-06-02') #Specify start and end time YYYY-MM_DD to control the time period of the cumulative sky
    
    
    'Albedo' #coming from the NASA Modis
    month_index = metdata.datetime[2].month - 1
    bsa = BSA_WSA_monthly.iloc[i,month_index*2]
    wsa = BSA_WSA_monthly.iloc[i,month_index*2+1]
    dni = np.array(metdata.dni)
    ghi = np.array(metdata.ghi)
    zenith = np.array(metdata.solpos['zenith'])
    albedo = wsa + (dni*np.cos(zenith*np.pi/180)/ghi)*(bsa-wsa)
    metdata.albedo = albedo
    demo.setGround(metdata.albedo)
    ...
    trackerdict = demo.set1axis(metdata = metdata, limit_angle = limit_angle, backtrack = backtrack, 
                            gcr = gcr, cumulativesky = cumulativesky, angledelta = angledelta) #was missing angledelta

    'Create Sky'
    trackerdict = demo.genCumSky1axis()

    'MyMetal'
    materialpav = 'My_Metal'
    Rrefl = 0.3
    Grefl = 0.25
    Brefl = 0.2
    demo.addMaterial(material=materialpav, Rrefl=Rrefl, Grefl=Grefl, Brefl=Brefl, specularity=0.03, roughness=0.15)
    ...
    if system_setup == 1:
        ...
        ygap = 0
        xgap = 0
        #REMEMBER to change number of modules to 22 otherwise they will be twice as long?
    ...
    if system_setup == 3:
        ...
        ygap = 0.145
        xgap = 0.0 # 0.027
        # Parameters are taken from Nicholas previous Master student


    module = demo.makeModule(name=moduletype, x=x, y=y,xgap=xgap, ygap=ygap, zgap=zgap, numpanels=numpanels)
    
    if TT_shape == 1:
        module.addTorquetube(diameter=0.145, material='My_Metal', tubetype='Square') # New torquetube generation function
    ...
    'Scene'
    hub_height = 2.55-0.115-(0.5*0.145)
    nMods = 44 #22
    if system_setup == 1:
        nMods = 22
        hub_height = 1.65-0.115-(0.5*0.145)
    if system_setup == 3:
        hub_height = 2.61-0.115-(0.5*0.145)
    nRows = 5
    ...
    'Create Oct files' #Combining Ground, Sky and Scene
    ...
    if system_setup == 1:
        ...
        #sensorsy = 11
    rowWanted = 3
    customname = f'_Row_{rowWanted}_Module_{modWanted}' # This is useful if we want to do various analysis.
       
    trackerdict = demo.analysis1axis(trackerdict, modWanted=modWanted, rowWanted = rowWanted, customname=customname, sensorsy=sensorsy) # default number of sensors y is 9
    
    #copy and save folder under the right name....
    source = '../lat_lon1/results'
    destination = f'../TT3_Set1/lat{Stations["lat"][i]}_lon{Stations["lon"][i]}_results'
    if os.path.exists(destination):
        shutil.rmtree(destination)
    copying = shutil.copytree(source,destination)
    
    if not os.path.exists(os.path.abspath('../albedo')  ):
        os.makedirs(os.path.abspath('../albedo'))
    albedo_df = pd.DataFrame(albedo)
    albedo_df.to_csv(f'../albedo/albedo_lat{Stations["lat"][i]}_lon{Stations["lon"][i]}.csv')
