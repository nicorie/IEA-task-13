...
path = r'C:\Users\NRI\Bifacial_Radiance_Files\results\GenCumSky_1deg'
...
rpoa_tot = results.groupby(['lat', 'lon', 'sensor'])[
    ['Wm2Back', 'Wm2Front']].sum()/1000

rpoa_tot_tt = results_tt.groupby(['lat', 'lon', 'sensor'])[
    ['Wm2Back', 'Wm2Front']].sum()/1000

rpoa_mean = rpoa_tot.groupby(['lat', 'lon'])[['Wm2Back', 'Wm2Front']].mean()
...
group = rpoa_tot.groupby(['lat', 'lon'])
...
results2.columns = ['lat', 'lon', 'x1', 'x2']  # 'x3', 'x4', 'x5']
...
colors = ['black', 'red', 'green', 'blue', 'orange']  # colors for lat/lon
...
for i, k in enumerate(keys):
    ...
    df.reset_index(inplace=True)
    df_tt = rpoa_tt.loc[(rpoa_tt['lat'] == k[0]) & (rpoa_tt['lon'] == k[1])]
