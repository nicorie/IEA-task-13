...
for t in tracking_angles.index:
    ...
    if istr_right in trackerkeys:
        theta = tracking_angles_s5.loc[t]['tracker_theta']
        surf_azm = tracking_angles_s5.loc[t]['surface_azimuth']
...
name = "ground_plane_10E"
text = "! genbox standard sloped_plane 200 200 0.1 | xform -ry 5.71 -t -113.1 -116.3 11.3"
...
for t in trackerkeys_sub:
    ...
    txt = f.read()
    txt1 = txt.replace(h, str(hub_height + 2*pitch*0.1))
    txt1 = txt1.replace('-0.0', str(2*pitch))  # move the y-origin

    txt2 = txt.replace(h, str(hub_height + 1*pitch*0.1))
    txt2 = txt2.replace('-0.0', str(pitch))

    txt3 = txt.replace(h, str(hub_height + -1*pitch*0.1))
    txt3 = txt3.replace('-0.0', str(-pitch))

    txt4 = txt.replace(h, str(hub_height + -2*pitch*0.1))
...
# trackerdict_sub['2022-01-03_1600']['scene'].showScene()
