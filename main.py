...
def _missingKeyWarning(dictype, missingkey, newvalue):  # prints warnings
    if type(newvalue) is bool:
        ...
        valueunit = 'm'
    print("Warning: {} Dictionary Parameters passed, but {} is missing. ".format(
        dictype, missingkey))
...
def _popen(cmd, data_in, data_out=PIPE):
    ...
    p = Popen(cmd, bufsize=-1, stdin=PIPE,
              stdout=data_out, stderr=PIPE, shell=shell)
    ...
    if err:
        if data:
            returntuple = (data.decode('latin1'), 'message: ' +
                           err.decode('latin1').strip())
        else:
            returntuple = (None, 'message: '+err.decode('latin1').strip())
    else:
        if data:
            # Py3 requires decoding
            returntuple = (data.decode('latin1'), None)
        else:
...
def _subhourlydatatoGencumskyformat(gencumskydata, label='right'):
    ...
    gencumskydata = gencumskydata.resample(
        '60T', closed='right', label='right').mean()
    ...
    padstart = pd.to_datetime(
        '%s-%s-%s %s:%s' % (gencumskydata.index.year[0], 1, 1, 1, 0)).tz_localize(tzinfo)
    padend = pd.to_datetime(
        '%s-%s-%s %s:%s' % (gencumskydata.index.year[0]+1, 1, 1, 0, 0)).tz_localize(tzinfo)
    # set first datapt to zero to forward fill w zeros
    gencumskydata.iloc[0] = 0
    # set last datapt to zero to forward fill w zeros
    gencumskydata.iloc[-1] = 0
    ...
    if any(gencumskydata.index.isin([padstart])):
        ...
        gencumskydata = pd.concat(
            [gencumskydata, pd.DataFrame(index=[padstart])])
    if any(gencumskydata.index.isin([padend])):
        ...
        gencumskydata = pd.concat(
            [gencumskydata, pd.DataFrame(index=[padend])])
...
class RadianceObj:
    ...
    def __init__(self, name=None, path=None, hpc=False):
        ...
        # HPC simulation is being run. Some read/write functions are modified
        self.hpc = hpc
    ...
    def addMaterial(self, material, Rrefl, Grefl, Brefl, materialtype='plastic',
        ...
        if (found and rewrite) or (not found):
            ...
            if materialtype == 'glass' or materialtype == 'mirror':
                ...
                file_object.write("\n0\n0\n5 {} {} {} {} {}".format(
                    Rrefl, Grefl, Brefl, specularity, roughness))
            file_object.close()
    ...
    def returnMaterialFiles(self, material_path=None):
        ...
        materialfilelist = [os.path.join(material_path, f)
    ...
    def getEPW(self, lat=None, lon=None, GetAll=False):
        ...
        def _returnEPWnames():
            ...
            r = requests.get(
                'https://github.com/NREL/EnergyPlus/raw/develop/weather/master.geojson', verify=False)
            ...
            for location in data['features']:
                match = re.search(
                    r'href=[\'"]?([^\'" >]+)', location['properties']['epw'])
                if match:
                    url = match.group(1)
                    ...
                    dftemp = pd.DataFrame(
                        {'url': [url], 'lat': [lattemp], 'lon': [lontemp], 'name': [name]})
        ...
        def _findClosestEPW(lat, lon, df):
            ...
            errorvec = np.sqrt(np.square(df.lat - lat) +
                               np.square(df.lon - lon))
    ...
    def readWeatherFile(self, weatherFile=None, starttime=None,
        ...
        if weatherFile is None:
            if hasattr(self, 'epwfile'):
                ...
                try:
                    weatherFile = _interactive_load(
                        'Select EPW or TMY3 climate file')
                except:
        ...
        def _tz_convert(metdata, metadata, tz_convert_val):
            ...
            if (type(tz_convert_val) == int) | (type(tz_convert_val) == float):
                ...
                metdata = metdata.tz_convert(
                    pytz.FixedOffset(tz_convert_val*60))
    ...
    def _saveTempTMY(self, tmydata, filename=None, starttime=None, endtime=None,
        ...
        if len(tmydata) == 8760:
            ...
            with warnings.catch_warnings():
                ...
                tmydata.index.values[:] = tmydata.index[:] + \
                    pd.DateOffset(year=(coerce_year))
                # Correcting last index to next year.
                tmydata.index.values[-1] = tmydata.index[-1] + \
                    pd.DateOffset(year=(coerce_year+1))
            ...
            if len(tmydata.index.year.unique()) == 1:
                ...
                if l != list(range(min(l), max(l)+1)):
                    ...
                    if filterdates is not None:
                        print("Filtering dates")
                        # Reducing years potentially
                        tmydata = tmydata[filterdates]
                    ...
                    if len(tmydata.index.year.unique()) == 1:
                        ...
                        gencumdict = [g for n, g in tmydata.groupby(
                            pd.Grouper(freq='Y'))]
                        ...
                        # clearing so that the dictionary style can be activated.
                        gencumskydata = None
        ...
        if gencumskydata is not None:
            ...
            print('Saving file {}, # points: {}'.format(
                csvfile, gencumskydata.__len__()))
            gencumskydata.to_csv(csvfile, index=False,
                                 header=False, sep=' ', columns=['GHI', 'DHI'])
        ...
        if gencumdict is not None:
            ...
            for ii in range(0, len(gencumdict)):
                ...
                print('Saving file {}, # points: {}'.format(
                    csvfile, gencumskydata.__len__()))
                gencumskydata.to_csv(
                    csvfile, index=False, header=False, sep=' ', columns=['GHI', 'DHI'])
    ...
    def getSingleTimestampTrackerAngle(self, metdata, timeindex, gcr=None,
        ...
        # bifacial_radiance uses East (morning) theta as positive
        tracker_theta = tracker_theta*-1
    ...
    def gendaylit(self, timeindex, metdata=None, debug=False):
        ...
        try:
            if ground.ReflAvg.shape == metdata.dni.shape:
                ...
                warnings.warn(
                    "Shape of ground Albedos and TMY data do not match.")
                return
        except:
        ...
        if debug is True:
            print(
                'Sky generated with Gendaylit, with DNI: %0.1f, DHI: %0.1f' % (dni, dhi))
        ...
        if sunalt <= 0:
            # reverse engineer elevation from ghi, dhi, dni
            sunalt = np.arcsin((ghi-dhi)/(dni+.001))*180/np.pi
        ...
        skyname = os.path.join(
            sky_path, "sky2_%s_%s_%s.rad" % (lat, lon, filename))
    ...
    def gendaylit2manual(self, dni, dhi, sunalt, sunaz):
        ...
        print('Sky generated with Gendaylit 2 MANUAL, with DNI: %0.1f, DHI: %0.1f' % (
            dni, dhi))
        ...
        skyStr = ("# start of sky definition for daylighting studies\n" +
            ...
            self.ground._makeGroundString(
                index=groundindex, cumulativesky=False)
    ...
    def gendaylit1axis(self, metdata=None, trackerdict=None, startdate=None,
        ...
        for i in range(0, len(trackerdict.keys())):
            ...
            if (metdata.ghi[i] > 0) & (~np.isnan(metdata.tracker_theta[i])):
                skyfile = self.gendaylit(
                    metdata=metdata, timeindex=i, debug=debug)
    ...
    def genCumSky1axis(self, trackerdict=None):
        ...
        for theta in sorted(trackerdict):
            ...
            # prefix for .cal file and skies\*.rad file
            savefile = '1axis_%s' % (theta)
            skyfile = self.genCumSky(
                gencumsky_metfile=csvfile, savefile=savefile)
    ...
    def makeOct1axis(self, trackerdict=None, singleindex=None, customname=None):
        ...
        # run through either entire key list of trackerdict, or just a single value
        for index in sorted(indexlist):
            try:
                filelist = self.materialfiles + \
                    [trackerdict[index]['skyfile'], trackerdict[index]['radfile']]
    ...
    def makeCustomObject(self, name=None, text=None):
        ...
        # update in 0.2.3 to shorten radnames
        customradfile = os.path.join('objects', '%s.rad' % (name))
        # py2 and 3 compatible: binary write, encode text first
    ...
    def printModules(self):
        ...
        print('Available module names: {}'.format(
            [str(x) for x in modulenames]))
        return modulenames
    ...
    def makeScene1axis(self, trackerdict=None, module=None, sceneDict=None,
        ...
        if use_clearanceheight:
            simplefix = 0
            # Not really, but this is the fastest
        ...
        if cumulativesky is True:        # cumulativesky workflow
            ...
            print('\nMaking ~%s .rad files for gendaylit 1-axis workflow (this takes a minute..)' %
                  (len(trackerdict)))
            count = 0
    ...
    def analysis1axis(self, trackerdict=None, singleindex=None, accuracy='low',
        ...
        for index in trackerkeys:   # either full list of trackerdict keys, or single index
            ...
            try:  # look for missing data
                ...
                analysis.analysis(octfile=octfile, name=name, frontscan=frontscanind,
                                  backscan=backscanind, accuracy=accuracy)
                trackerdict[index]['AnalysisObj'] = analysis
            except Exception as e:  # problem with file. TODO: only catch specific error types here.
                warnings.warn(
                    'Index: {}. Problem with file. Error: {}. Skipping'.format(index, e), Warning)
                return
            ...
            try:  # on error, trackerdict[index] is returned empty
                ...
                warnings.warn(
                    'Index: {}. Trackerdict key not found: {}. Skipping'.format(index, e), Warning)
                return
        ...
        if np.sum(self.Wm2Front) == 0:
            # these are accumulated over all indices passed in.
        ...
        if singleindex is None:
            ...
            if self.cumulativesky is True:
                ...
                temptrackerdict = trackerdict[list(
                    trackerdict)[0]]['AnalysisObj']
                ...
                cumanalysisobj._saveResultsCumulative(
                    frontcum, rearcum, savefile=cumfilename)
                ...
                try:
                    ...
                    frontcum['y'] = y
                    frontcum['z'] = z
                    frontcum['mattype'] = trackerdict[trackerkeys[0]
                                                      ]['AnalysisObj'].mattype
                    frontcum['Wm2'] = self.Wm2Front
                    rearcum['x'] = x
                    rearcum['y'] = y
                    rearcum['z'] = rearz
                    rearcum['mattype'] = trackerdict[trackerkeys[0]
                                                     ]['AnalysisObj'].rearMat
                    rearcum['Wm2'] = self.Wm2Back
                    print("\nSaving Cumulative results")
                    cumanalysisobj._saveResultsCumulative(
...
class GroundObj:
    ...
    def printGroundMaterials(self, materialString=None):
        ...
        Rreflall = []
        Greflall = []
        Breflall = []  # RGB material reflectance
        temp = f.read().split()
        f.close()
        ...
        for i in index:
            ...
            # RGB reflectance comes a few more down the list
            Rreflall.append(float(temp[i+5]))
...
class SceneObj:
    ...
    def __init__(self, module=None, name=None):
        ...
        # default False.  Set True by makeScene after sceneobj created.
        self.hpc = False
        if name is None:
            self.name = 'Scene0'
        else:
    ...
    def _makeSceneNxR(self, modulename=None, sceneDict=None, radname=None):
        ...
        text += '-a %s -t %s 0 0 -a %s -t 0 %s 0 ' % (
            nMods, self.module.scenex, nRows, pitch)
...
#        self.hub_height = hubheight
    ...
    def saveImage(self, filename=None, view=None):
        ...
        if err:
            ...
            print(
                f"Scene image saved: images/{filename}_{view.replace('.vp','')}.hdr")
...
class MetObj:
    ...
    def __init__(self, tmydata, metadata, label='right'):
        ...
        try:
            self.meastracker_angle = np.array(
                tmydata['Tracker Angle (degrees)'])
        except KeyError:
        ...
        try:  # make sure the data is tz-localized.
            datetimetz = datetimetz.tz_localize(pytz.FixedOffset(
                self.timezone*60))  # use pytz.FixedOffset (in minutes)
        # data is tz-localized already. Just put it in local time.
        except TypeError:
            datetimetz = datetimetz.tz_convert(
                pytz.FixedOffset(self.timezone*60))
        # check for data interval. default 1h.
        try:
            interval = datetimetz[1]-datetimetz[0]
        except IndexError:
            # ISSUE: if 1 datapoint is passed, are we sure it's hourly data?
        ...
        if label.lower() == 'center':
            ...
            sunup = pvlib.irradiance.solarposition.sun_rise_set_transit_spa(
                datetimetz, lat, lon)  # new for pvlib >= 0.6.1
            ...
            if interval == pd.Timedelta('1h'):
                ...
                if label.lower() == 'right':
                    ...
                    sunup = pvlib.irradiance.solarposition.sun_rise_set_transit_spa(
                        datetimetz, lat, lon)  # new for pvlib >= 0.6.1
                    # default sun angle 30 minutes before timestamp
                    sunup['minutedelta'] = int(interval.seconds/2/60)
                    # vector update of minutedelta at sunrise
                    sunrisemask = sunup.index.hour - \
                        1 == sunup['sunrise'].dt.hour
                    sunup['minutedelta'].mask(sunrisemask, np.floor(
                        (60-(sunup['sunrise'].dt.minute))/2), inplace=True)
                    # vector update of minutedelta at sunset
                    sunsetmask = sunup.index.hour-1 == sunup['sunset'].dt.hour
                    sunup['minutedelta'].mask(sunsetmask, np.floor(
                        (60-(sunup['sunset'].dt.minute))/2), inplace=True)
                    # save corrected timestamp
                    sunup['corrected_timestamp'] = sunup.index - \
                        pd.to_timedelta(sunup['minutedelta'], unit='m')
                    ...
                    sunup = pvlib.irradiance.solarposition.sun_rise_set_transit_spa(
                        datetimetz, lat, lon)
                    # default sun angle 30 minutes after timestamp
                    sunup['minutedelta'] = int(interval.seconds/2/60)
                    # vector update of minutedelta at sunrise
                    sunrisemask = sunup.index.hour == sunup['sunrise'].dt.hour
                    sunup['minutedelta'].mask(sunrisemask, np.ceil(
                        (60+sunup['sunrise'].dt.minute)/2), inplace=True)
                    # vector update of minutedelta at sunset
                    sunsetmask = sunup.index.hour == sunup['sunset'].dt.hour
                    sunup['minutedelta'].mask(sunsetmask, np.ceil(
                        (60+sunup['sunset'].dt.minute)/2), inplace=True)
                    # save corrected timestamp
                    sunup['corrected_timestamp'] = sunup.index + \
                        pd.to_timedelta(sunup['minutedelta'], unit='m')
                    ...
                    raise ValueError(
                        'Error: invalid weather label passed. Valid inputs: right, left or center')
            else:
                ...
                sunup = pvlib.irradiance.solarposition.sun_rise_set_transit_spa(
                    datetimetz, lat, lon)  # new for pvlib >= 0.6.1
                sunup['corrected_timestamp'] = sunup.index - \
                    pd.Timedelta(minutes=minutedelta)

        self.solpos = pvlib.irradiance.solarposition.get_solarposition(
            sunup['corrected_timestamp'], lat, lon, elev)
    ...
    def _set1axis(self, azimuth=180, limit_angle=45, angledelta=None,
        ...
        # track whether we're using cumulativesky or gendaylit
        self.cumulativesky = cumulativesky
    ...
    def _getTrackingAngles(self, azimuth=180, limit_angle=45,
        ...
        self.tracker_theta = np.round(
            trackingdata['tracker_theta'], 2).tolist()
        self.surface_tilt = np.round(trackingdata['surface_tilt'], 2).tolist()
        self.surface_azimuth = np.round(
            trackingdata['surface_azimuth'], 2).tolist()
        ...
        # this has the original time data in it
        trackingdata.index = self.sunrisesetdata.index
    ...
    def _makeTrackerCSV(self, theta_list, trackingdata):
        ...
        for theta in sorted(trackerdict):
            ...
            datetimetemp = tempdata.index.strftime(
...
class AnalysisObj:
    ...
    def _saveResults(self, data=None, reardata=None, savefile=None, RGB=False):
        ...
        if RGB:
            data_sub = {key: data[key] for key in [
                'x', 'y', 'z', 'mattype', 'Wm2', 'r', 'g', 'b']}
        else:
            data_sub = {key: data[key]
                        for key in ['x', 'y', 'z', 'mattype', 'Wm2']}
        ...
        if reardata is not None:
            ...
            df.insert(8, 'Back/FrontRatio',
                      df['Wm2Back'] / (df['Wm2Front']+.001))
        ...
        if rearswapflag:
            df = df.rename(
                columns={'Wm2Front': 'Wm2Back', 'mattype': 'rearMat'})
    ...
    def _saveResultsCumulative(self, data, reardata=None, savefile=None):
        ...
        if reardata is not None:
            ...
            # add 1mW/m2 to avoid dividebyzero
            self.backRatio = [x/(y+.001)
                              for x, y in zip(reardata['Wm2'], data['Wm2'])]
    ...
    def moduleAnalysis(self, scene, modWanted=None, rowWanted=None,
       ...
       # offset = moduleDict['offsetfromaxis']
        ...
        if hasattr(scene.module, 'z'):
            ...
            print(
                "Module's z not set on sceneDict internal dictionary. Setting to default")
            modulez = 0.02
        ...
        x3 = (offset + modulez + frontsurfaceoffset) * \
            np.sin(tilt*dtor) * np.sin((azimuth)*dtor)
        y3 = (offset + modulez + frontsurfaceoffset) * \
            np.sin(tilt*dtor) * np.cos((azimuth)*dtor)
        z3 = (offset + modulez + frontsurfaceoffset) * np.cos(tilt*dtor)

        # Axis of rotation Offset, for the back of the module
        x4 = (offset - backsurfaceoffset) * \
            np.sin(tilt*dtor) * np.sin((azimuth)*dtor)
        y4 = (offset - backsurfaceoffset) * \
            np.sin(tilt*dtor) * np.cos((azimuth)*dtor)
        ...
        if ((getattr(scene.module, 'cellModule', None)) and
            ...
            xinc_back = -((sceney - ycell) / (scene.module.cellModule.numcellsy-1)
                          ) * np.cos((tilt)*dtor) * np.sin((azimuth)*dtor)
            yinc_back = -((sceney - ycell) / (scene.module.cellModule.numcellsy-1)
                          ) * np.cos((tilt)*dtor) * np.cos((azimuth)*dtor)
            zinc_back = (
                (sceney - ycell) / (scene.module.cellModule.numcellsy-1)) * np.sin(tilt*dtor)
            firstsensorxstartfront = xstartfront - scene.module.cellModule.ycell / \
                2 * np.cos((tilt)*dtor) * np.sin((azimuth)*dtor)
            firstsensorxstartback = xstartback - ycell/2 * \
                np.cos((tilt)*dtor) * np.sin((azimuth)*dtor)
            firstsensorystartfront = ystartfront - ycell/2 * \
                np.cos((tilt)*dtor) * np.cos((azimuth)*dtor)
            firstsensorystartback = ystartback - ycell/2 * \
                np.cos((tilt)*dtor) * np.cos((azimuth)*dtor)
            ...
            xinc_back = -(sceney/(sensorsy_back + 1.0)) * \
                np.cos((tilt)*dtor) * np.sin((azimuth)*dtor)
            yinc_back = -(sceney/(sensorsy_back + 1.0)) * \
                np.cos((tilt)*dtor) * np.cos((azimuth)*dtor)
            zinc_back = (sceney/(sensorsy_back + 1.0)) * np.sin(tilt*dtor)

            if sensors_diff:
                xinc_front = -(sceney/(sensorsy_front + 1.0)) * \
                    np.cos((tilt)*dtor) * np.sin((azimuth)*dtor)
                yinc_front = -(sceney/(sensorsy_front + 1.0)) * \
                    np.cos((tilt)*dtor) * np.cos((azimuth)*dtor)
                zinc_front = (sceney/(sensorsy_front + 1.0)) * \
                    np.sin(tilt*dtor)
            ...
            if sensorsx_back > 1.0:
                sx_xinc_back = -(x/(sensorsx_back*1.0+1)) * \
                    np.cos((azimuth)*dtor)
                sx_yinc_back = (x/(sensorsx_back*1.0+1)) * \
                    np.sin((azimuth)*dtor)
                # Not needed unless axis_tilt != 0, which is not a current option
                sx_zinc_back = 0.0

                firstsensorxstartback = firstsensorxstartback + \
                    (x/2.0) * np.cos((azimuth)*dtor) + sx_xinc_back
                firstsensorystartback = firstsensorystartback - \
                    (x/2.0) * np.sin((azimuth)*dtor) + sx_yinc_back
            ...
            if sensorsx_front > 1.0:
                sx_xinc_front = -(x/(sensorsx_front*1.0+1)) * \
                    np.cos((azimuth)*dtor)
                sx_yinc_front = (x/(sensorsx_front*1.0+1)) * \
                    np.sin((azimuth)*dtor)
                # Not needed unless axis_tilt != 0, which is not a current option
                sx_zinc_front = 0.0

                firstsensorxstartfront = firstsensorxstartfront + \
                    (x/2.0) * np.cos((azimuth)*dtor) + sx_xinc_back
                firstsensorystartfront = firstsensorystartfront - \
                    (x/2.0) * np.sin((azimuth)*dtor) + sx_yinc_back
        ...
        if debug is True:
            ...
            print("Final Start Coordinate Front",
                  xstartfront, ystartfront, zstartfront)
        ...
        if modscanfront is not None:
            frontscan2 = _modDict(originaldict=frontscan,
                                  moddict=modscanfront, relative=relative)
        else:
            frontscan2 = frontscan.copy()
        if modscanback is not None:
            backscan2 = _modDict(originaldict=backscan,
                                 moddict=modscanback, relative=relative)
        else:
    ...
    def analyzeRow(self, octfile, scene, rowWanted=None, name=None,
        ...
        for i in range(nMods):
            ...
            frontscan, backscan = self.moduleAnalysis(scene, sensorsy=sensorsy,
                                                      ...
                                                      rowWanted=rowWanted)
            allscan = self.analysis(
                octfile, name+'_Module_'+str(i), frontscan, backscan)
            ...
            temp_dict['Back/FrontRatio'] = list(
                np.array(front_dict['Wm2'])/np.array(back_dict['Wm2']))
    ...
    def analysis(self, octfile, name, frontscan, backscan,
        ...
        if frontDict is not None:
            if len(frontDict['Wm2']) != len(backDict['Wm2']):
                ...
                self._saveResults(frontDict, reardata=None,
                                  savefile='irr_%s.csv' % (name+'_Front'), RGB=RGB)
                self._saveResults(data=None, reardata=backDict,
                                  savefile='irr_%s.csv' % (name+'_Back'), RGB=RGB)
            else:
                self._saveResults(frontDict, backDict,
                                  'irr_%s.csv' % (name), RGB=RGB)

        return frontDict, backDict

    def modifyLineScan(self, linepts_back, linepts_front, sensorsy, trackerdict,
                       add_outboard=True):
        """
        Custom function by N. Riedel-Lyngskær and N. Andersen to place a 
        sensor on the bottom of the torque tube, and optionally, two sensors
        at a north and south outboard position 0.75m away from the edge modules

        if add_outboard is set to True, then the user ought to ensure that the 
        torque tube has been extended sufficiently.

        Parameters
        ----------
        linepts_back : str
            x,y,z and orientations. Typically output from _linePtsMakeDict
        linepts_front : str
            x,y,z and orientations. Typically output from _linePtsMakeDict.
        sensorys : int
            number of sensors used in the linescan.
        trackerdict : dictionary with single key (trackerdict[t])
            trackerdict[t] after makeScene1axis is run
        add_outboard : boolean, optional
            add sensors 0.75 m north and south of the string. Default=True.

        Returns
        -------
        modified_linepts_back : str
            linepts_back with the updated sensor positions.
        modified_linepts_front : str
            linepts_front with the updated sensor positions.

        """
        linepts_str = str(linepts_back.encode())
        linepts_strip = linepts_str.strip('b').strip('\'')
        linepts_split = linepts_strip.split('\\r')

        tt = int(np.floor(sensorsy/2))
        left = int(sensorsy-1)

        old_pt = linepts_split[tt].split(' ')
        left_pt = linepts_split[left].split(' ')
        vector_x = float(left_pt[0]) - float(old_pt[0])  # change in x
        vector_z = float(left_pt[2]) - float(old_pt[2])  # change in z

        x = sp.symbols('x')
        zgap = trackerdict['scene'].module.zgap
        tt_diam = trackerdict['scene'].module.torquetube.diameter
        radius = tt_diam+zgap

        eq1 = sp.Eq((x*vector_x)**2+(x*vector_z)**2-radius**2, 0)
        sol = sp.solve(eq1)

        sol = [i for i in sol if i > 0]

        new_pt_x = float(old_pt[0]) + sol[0] * (-vector_z)
        new_pt_z = float(old_pt[2]) + sol[0] * vector_x
        linepts_split2 = linepts_split[10].split(
            ' ')  # break down the middle point

        linepts_split[tt] = str(new_pt_x) + ' ' + linepts_split2[1] + ' ' + str(new_pt_z) + \
            ' ' + linepts_split2[3] + ' ' + \
            linepts_split2[4] + ' ' + linepts_split2[5] + ' '

        modified_linepts_back = linepts_split
        modified_linepts_front = linepts_front

        if add_outboard:
            outboard = modified_linepts_back[tt]
            scenex = trackerdict['scene'].module.scenex
            nMods = trackerdict['scene'].sceneDict['nMods']
            # analysis should check latitude if north or south hemisphere
            outboard_y_s = -scenex * (nMods-1)/2 - 0.75  # south
            outboard_y_n = scenex * (nMods-1)/2 + 0.75  # north

            outboard_split = outboard.split(' ')

            outboard_line_s = str(outboard_split[0]) + ' ' + str(outboard_y_s) + ' ' + str(outboard_split[2]) + \
                ' ' + str(outboard_split[3]) + ' ' + \
                str(outboard_split[4]) + ' ' + str(outboard_split[5]) + ' '

            outboard_line_n = str(outboard_split[0]) + ' ' + str(outboard_y_n) + ' ' + str(outboard_split[2]) + \
                ' ' + str(outboard_split[3]) + ' ' + \
                str(outboard_split[4]) + ' ' + str(outboard_split[5]) + ' '

            # add an additional outboard points to the frontscan, so back and front have same length
            outboard_line_front_s = str(outboard_split[0]) + ' ' + str(outboard_y_s) + ' ' + str(outboard_split[2]) + \
                ' ' + str(-1*float(outboard_split[3])) + ' ' + \
                str(-1*float(outboard_split[4])) + ' ' + \
                str(-1*float(outboard_split[5])) + ' '

            outboard_line_front_n = str(outboard_split[0]) + ' ' + str(outboard_y_n) + ' ' + str(outboard_split[2]) + \
                ' ' + str(-1*float(outboard_split[3])) + ' ' + \
                str(-1*float(outboard_split[4])) + ' ' + \
                str(-1*float(outboard_split[5])) + ' '

            linepts_string_front = str(linepts_front.encode())
            linepts_strip_front = linepts_string_front.strip('b').strip('\'')
            linepts_split_front = linepts_strip_front.split('\\r')

            linepts_split.append(outboard_line_s)
            linepts_split.append(outboard_line_n)
            linepts_split_front.append(outboard_line_front_s)
            linepts_split_front.append(outboard_line_front_n)

            line_pts = ""
            for line in range(0, len(linepts_split)):
                line_pts = line_pts + linepts_split[line] + "\r"

            line_pts_front = ""
            for line in range(0, len(linepts_split_front)):
                line_pts_front = line_pts_front + \
                    linepts_split_front[line] + "\r"

            modified_linepts_back, modified_linepts_front = line_pts, line_pts_front

        return modified_linepts_back, modified_linepts_front
...
def quickExample(testfolder=None):
    ...
    demo = bifacial_radiance.RadianceObj(
        'simple_panel', path=testfolder)  # Create a RadianceObj 'object'

    # input albedo number or material name like 'concrete'.
    # To see options, run setGround without any input.
    demo.setGround(0.62)
    try:
        # pull TMY data for any global lat/lon
        epwfile = demo.getEPW(lat=40.01667, lon=-105.25)
    ...
    if cumulativeSky:
        ...
        timeindex = metdata.datetime.index(
            pd.to_datetime('2001-06-17 12:0:0 -7'))
